...
console.log(start)
  ...
  while (state.pos < max) {
    ...
    if (state.src.charCodeAt(state.pos) === leftSquareBracketUTF8/* [ */  && state.src.charCodeAt(state.pos +1) === cUTF8) {
      let string9 = state.src.slice(state.pos, state.pos + 9)
      ...
      if (stringMatch.bool) {
        ...
        let stringName = stringMatch.string.slice(regexIndexOf(stringMatch.string, /\[/g).firstIndex, regexIndexOf(stringMatch.string, /\]/g).firstIndex + 1)
        let className = stringMatch.string.slice(regexIndexOf(stringMatch.string, /\[/g).firstIndex + 1, regexIndexOf(stringMatch.string, /\]/g).firstIndex)
  ...
  token.markup  = match.string.slice(1, match.string.length-1);
  token.attrs = [['class', match.string.slice(1, match.string.length-1)]]
...
module.exports = function sub_plugin(md) {
  md.inline.ruler.after('emphasis', 'css', subscript);
};
